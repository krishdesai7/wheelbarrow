...
complete, buildable Python project to disk:
    ...
    <root>/
      ...
      src/<module>/
        __init__.py
        __main__.py
        bin/<binary>      <- the staged executable, mode 0o755
"""
...
def scaffold_project(spec: PackageSpec, binary: Path, root: Path) -> Path:
    ...
    stage_binary(binary, bin_dir / spec.binary_name)
    return root
