...
class TestElf:
    ...
    def test_architecture(self, write_binary, machine, expected):
...
class TestMachO:
    ...
    def test_java_class_file_is_not_mistaken_for_fat_macho(self, write_binary):
...
class TestPe:
    ...
    def test_mz_without_pe_signature(self, write_binary):
...
class TestRejections:
    ...
    def test_shell_script_gets_a_useful_message(self, write_binary):
        path = write_binary("tool", b"#!/bin/sh\necho hi\n")
        with pytest.raises(InspectionError, match="is a script"):
            inspect_binary(path)

    def test_unrecognised_bytes(self, write_binary):
    ...
    def test_tiny_file(self, write_binary):
